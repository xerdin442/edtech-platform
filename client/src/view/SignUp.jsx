...
const SignUp = () => {
  ...
  const validateForm = () => {
    ...
    if (!schoolName || schoolName.length < 5 || schoolName.length > 100) {
      validationErrors.schoolName = (
        <span className="red-error">
          <img src={danger} alt="Error Icon" /> School name must be between 5 to 100 characters long
        </span>
      );
    }
    if (!schoolEmail) {
      ...
      if (!emailRegex.test(schoolEmail)) {
        validationErrors.schoolEmail = (
          <span className="red-error">
            <img src={danger} alt="Error Icon" /> Invalid email format
          </span>
        );
      }
    }
    if (!schoolPassword) {
      ...
      !/[a-z]/.test(schoolPassword) || 
      !/[A-Z]/.test(schoolPassword) || 
      !/[0-9]/.test(schoolPassword) || 
      !/[!@#$%^&*(),.?":{}|<>]/.test(schoolPassword)) {
      validationErrors.schoolPassword = (
        <span className="red-error">
          <img src={danger} alt="Error Icon" /> Password must be at least 8 characters long and contain one uppercase letter, one lowercase letter, one digit, and one symbol
        </span>
      );
    }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, form: errorMessage })); // Set a form-level error
      ...
      <div className="signUp-div">
        ...
        <div className="side-2">
          <section className="formSection">
            ...
            {errors.form && <p className="error-message">{errors.form}</p>} {/* Display form-level error */}
